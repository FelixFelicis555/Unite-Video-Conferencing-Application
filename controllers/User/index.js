...
console.log(query);
    ...
    try{
        const userRes=await User.findOne(query).populate("rooms");
        res.json({
...
const addRooms=async(req,res)=>{
    ...
    let room;
    try{
        room=await User.findOne({roomID:roomId});
        console.log(room);
    ...
    try{
        const user = await User.findOneAndUpdate({email},{$addToSet:{rooms:room._id}});
